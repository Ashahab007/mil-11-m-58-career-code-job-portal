...
const AddJobs = () => {
  // 27.4 as the user is recruiter so we use user email using defaultValue in email field so call the custom hook useAuth() to get the user data
  const { user } = useAuth();
  // 27.3 add event handler to get the data from the form
  const handleAddForm = (e) => {
    ...
    // 27.4 for similarity with the added data in the data base we have to process the requirements, responsibilities and salaryrange data.

    // process salary range
    // here destructured the min, max, currency, and ...newJob as the rest that is from now newJob doesn't contains min, max, currency data because we have to process it and add to newHJob later
    ...
    // 27.7 send the job application to db
    axios
      .post("http://localhost:3000/jobs", newJob)
      .then((res) => {
        console.log(res);
        // 27.8 Show with alert
